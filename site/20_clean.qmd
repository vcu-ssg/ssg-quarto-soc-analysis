...
TL;DR: Here is a link to the [cleaned dataframe](stacked_df.csv).  This dataframe contains one tuple per (TERM,CRN,INSTRUCTOR,MEETING_CODE).

TL;DR: Here is a link to the [google sheet](https://docs.google.com/spreadsheets/d/1ZK7k8M85CXLof6FdeJYJuGFbfjsOXrCv5mc7OgUInWw/edit#gid=1484314377) with the cleaned data loaded 
...
The new VCU report added new-line characters to a few of the
column names.  This code cleans them up.
...
These fields were dropped when the report changed layouts in Fall 2024.  Instructor
first name is good to have, so these fields were added back and can be remapped later.
...
def fix_names( id, note, old_name, new_name ):
    """ replace instructor old_name with instructor new_name """
    # find all sections with the old name, then update these sections.
    # You can't simply change old name to new name using old name as key!

    section_terms = sections_df[
        (sections_df["PRIMARY INSTRUCTOR FIRST NAME"]==old_name["first_name"])&(sections_df["PRIMARY INSTRUCTOR LAST NAME"]==old_name["last_name"]) ]

    for i,section in section_terms.iterrows():
        fix_sections( id, note, section,
        {"PRIMARY INSTRUCTOR FIRST NAME":new_name["first_name"],
        "PRIMARY INSTRUCTOR LAST NAME":new_name["last_name"]})

    section_terms = sections_df[
        (sections_df["SECONDARY INSTRUCTOR FIRST NAME"]==old_name["first_name"])&(sections_df["SECONDARY INSTRUCTOR LAST NAME"]==old_name["last_name"]) ]

    for i,section in section_terms.iterrows():
        fix_sections( id, note, section,
        {"SECONDARY INSTRUCTOR FIRST NAME":new_name["first_name"],
        "SECONDARY INSTRUCTOR LAST NAME":new_name["last_name"]})
...
Here I'll add a few buffer columns, in case the structure of the report changes, I can add or delete columns
here to keep the spreadsheet from breaking.
