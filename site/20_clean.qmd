...
stacked_df["instructor_cnt"] = stacked_df.groupby(['TERM','COURSE','CRN','instructor'])['instructor'].transform('count') / stacked_df.groupby(['TERM','COURSE','CRN'])['CRN'].transform('count')
```
...
# Computer number of CRN that are sharing this room at the same time as this section.
stacked_df['shared_mtgs_cnt'] = stacked_df.groupby(['TERM','COURSE','CRN','meeting_id'])['meeting_id'].transform('count') / stacked_df.groupby(['TERM','COURSE','CRN'])['CRN'].transform('count')
...
stacked_df.loc[(stacked_df['wrkld_sample_type']=='LEC'),'wrkld_sample_note'] = "BASE: [wrkld] = 1.0 per CRN"
stacked_df.loc[(stacked_df['wrkld_sample_type']=='LEC')&(stacked_df['instructor_cnt']<1.0),'wrkld_sample_note'] = "BASE: [wrkld] = 0.5 per CRN (co-teaching)"
...
stacked_df.loc[stacked_df['COURSE'].isin(['ENGR497']),'wrkld_sample_type'] = "VIP"
...
stacked_df.loc[(stacked_df['MAX CREDITS']==4)&(stacked_df['wrkld_sample_type']=='LEC')&(stacked_df['mtgs_per_wk']==1),'wrkld_sample_type'] = 'LAB'
...
stacked_df.loc[stacked_df['wrkld_sample_type'].isin(['RES','IND','FLD']),'wrkld_sample_note'] = "RULE: [wrkld]=0.0 for INS, RES and FLD"
...
stacked_df.loc[stacked_df['wrkld_sample_type'].isin(['SEM']),'wrkld_sample_note'] = f"RULE: [wrkld] = [wrkld] / {seminars_per_lecture} for SEM"
stacked_df.loc[(stacked_df['wrkld_sample_type'].isin(['SEM']))&(stacked_df['instructor_cnt']<1.0),'wrkld_sample_note'] = f"RULE: [wrkld] = [wrkld] / {seminars_per_lecture} for SEM (co-teaching)"
...
stacked_df.loc[stacked_df['MODALITY CODE'].isin(['ROND']),'wrkld_sample_note'] = f"RULE: [wrkld] = 0 for ROND (online synchronous)"
